...
SAVEDS ULONG ASM LayoutFunc(REG(a0, UNUSED struct Hook *h),REG(a2, UNUSED Object *obj),REG(a1, struct MUI_LayoutMsg *lm))
{
	switch (lm->lm_Type)
	{
		case MUILM_MINMAX:
...
SAVEDS ULONG ASM PressFunc(REG(a0, UNUSED struct Hook *h),REG(a2, Object *app),REG(a1, LONG *num))
{
...
int main(UNUSED int argc, UNUSED char *argv[])
{
	...
	static struct Hook PressHook  = { {0,0}, PressFunc , NULL, NULL };
	ULONG signals;
	BOOL running = TRUE;
	int i;
	...
	app = ApplicationObject,
		...
		MUIA_Application_Version    , "$VER: Layout 20.165 (07.02.2009)",
