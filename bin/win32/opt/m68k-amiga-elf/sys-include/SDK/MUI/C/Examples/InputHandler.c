...
ULONG mNew(struct IClass *cl,Object *obj,Msg msg)
{
	...
	if ((data->port = AllocSysObjectTags(ASOT_PORT, TAG_DONE)) != NULL)
	{
		if ((data->req = (struct timerequest *)AllocSysObjectTags(ASOT_IOREQUEST,
			ASOIOR_Size, sizeof(*data->req),
			ASOIOR_ReplyPort, data->port,
			TAG_DONE)) != NULL)
		{
	#else
...
ULONG mDispose(struct IClass *cl,Object *obj,Msg msg)
{
	...
	if (data->req)
	{
		...
		#if defined(__amigaos4__)
		FreeSysObject(ASOT_IOREQUEST, data->req);
		#else
	...
	{
		#if defined(__amigaos4__)
		FreeSysObject(ASOT_PORT, data->port);
		#else
...
ULONG mTrigger(struct IClass *cl,Object *obj,UNUSED Msg msg)
{
	struct MyData *data = INST_DATA(cl,obj);
...
int main(UNUSED int argc, UNUSED char *argv[])
{
	...
	app = ApplicationObject,
		...
		MUIA_Application_Version    , "$VER: Class5 20.167 (07.02.2009)",
